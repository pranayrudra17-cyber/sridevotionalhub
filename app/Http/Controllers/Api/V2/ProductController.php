...
use App\Http\Resources\V2\PujariLangCollection;
...
class ProductController extends Controller
{
    ...
    {
        $products = Product::where('featured', 0)->physical();
        return new ProductMiniCollection(filter_products($products)->latest()->paginate(10));
    }

    public function astrology()
    {
        return Cache::remember('app.astrology', 86400, function(){
            $products = Product::where('digital', 1);
    ...
    {
        return Cache::remember('app.pooja', 86400, function(){
            $products = Product::where('digital', 2);
            return new ProductMiniCollection(filter_products($products)->limit(20)->get());
        });
    }
    ...
    public function astrologyRelated($id)
    {
        return Cache::remember("app.astrology_related_products-$id", 86400, function() use ($id){
            $product = Product::find($id);
    ...
    public function poojaRelated($id)
    {
        return Cache::remember("app.pooja_related_products-$id", 86400, function() use ($id){
            $product = Product::find($id);
            $products = Product::where('category_id', $product->category_id)->where('id', '!=', $id);
            return new ProductMiniCollection(filter_products($products)->limit(10)->get());
        });
    ...
    public function variantPrice(Request $request)
    {
        ...
        return response()->json([
            ...
            'description' => $product_stock->description == null ? "" : $product_stock->description
        ]);
    }
    ...
    public function poojariLang()
    {
        // return response()->json(PujariLang::where('status',1)->get());
        return new PujariLangCollection(PujariLang::where('status',1)->get());
    }
}
