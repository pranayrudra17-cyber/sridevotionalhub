...
class ShopController extends Controller
{
    ...
    public function allProducts($id)
    {
        ...
        return new ProductCollection(Product::where('user_id', $shop->user_id)->where('published',1)->where('digital',0)->latest()->paginate(10));
    }
    ...
    public function topSellingProducts($id)
    {
        ...
        return Cache::remember("app.top_selling_products-$id", 86400, function () use ($shop){
            return new ProductMiniCollection(Product::where('user_id', $shop->user_id)->where('published',1)->where('digital',0)->orderBy('num_of_sale', 'desc')->limit(10)->get());
        });
    }
    ...
    public function featuredProducts($id)
    {
        ...
        return Cache::remember("app.featured_products-$id", 86400, function () use ($shop){
            return new ProductMiniCollection(Product::where(['user_id' => $shop->user_id, 'featured' => 1])->where('digital',0)->where('published',1)->latest()->limit(10)->get());
        });
    }
    ...
    public function newProducts($id)
    {
        ...
        return Cache::remember("app.new_products-$id", 86400, function () use ($shop){
            return new ProductMiniCollection(Product::where('user_id', $shop->user_id)->where('published',1)->where('digital',0)->orderBy('created_at', 'desc')->limit(10)->get());
        });
    }
