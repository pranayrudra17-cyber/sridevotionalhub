...
class HomeController extends Controller
{
    ...
    public function index()
    {
        $featured_categories = Cache::rememberForever('featured_categories', function () {
            return Category::where('featured', 1)->where('digital', 0)->get();
        });
        ...
        $newest_products = Cache::remember('newest_products', 3600, function () {
            return filter_products(Product::latest()->where('digital', 0))->limit(12)->get();
        });

        // Pranay Rudra
        $astrology_products = Cache::remember('astrology_products', 3600, function () {
            return (Product::where('published', 1)->where('digital', 1))->limit(12)->get();
        });
        $pooja_products = Cache::remember('pooja_products', 3600, function () {
            return (Product::where('published', 1)->where('digital', 2))->limit(12)->get();
        });
        // end

        return view('frontend.index', compact('featured_categories', 'todays_deal_products', 'newest_products', 'astrology_products', 'pooja_products'));
    }
    ...
    {
        $detailedProduct  = Product::physical()->with('reviews', 'brand', 'stocks', 'user', 'user.shop')->where('auction_product', 0)->where('slug', $slug)->where('approved', 1)->first();
    ...
    {
        $categories = Category::where('level', 0)->where('digital', 0)->orderBy('order_level', 'desc')->get();
    ...
    public function astrology(Request $request)
    {
        ...
        $conditions = array_merge($conditions, ['digital' => 1]);
        $products = Product::where($conditions);
        $products = filter_products($products)->with('taxes')->paginate(120);
        // dump($products);
        return view('frontend.astrology', compact('products'));
    }

    public function astrology_slug(Request $request, $slug)
    {
        $detailedProduct  = Product::with('reviews', 'brand', 'stocks', 'user', 'user.shop')->where('auction_product', 0)->where('digital', 1)->where('slug', $slug)->where('approved', 1)->first();
        $product_queries = ProductQuery::where('product_id', $detailedProduct->id)->where('customer_id', '!=', Auth::id())->latest('id')->paginate(10);
        $total_query = ProductQuery::where('product_id', $detailedProduct->id)->count();
        // Pagination using Ajax
        // if (request()->ajax()) {
        ...
        if ($detailedProduct != null && $detailedProduct->published) {
            $langs = PujariLang::where('status',1)->get();
            return view('frontend.astrology_product_details', compact('detailedProduct', 'product_queries', 'total_query', 'langs'));
        }
        else {
            abort(404);
        }
    ...
    public function pooja(Request $request)
    {
        // Pranay Rudra
        $conditions = ['published' => 1];
        $conditions = array_merge($conditions, ['digital' => 2]);
        $products = Product::where($conditions);
        $products = filter_products($products)->with('taxes')->paginate(120);
        // dump($products);
        return view('frontend.pooja', compact('products'));
    }

    public function pooja_slug(Request $request, $slug)
    {
        $detailedProduct  = Product::with('reviews', 'brand', 'stocks', 'user', 'user.shop')->where('auction_product', 0)->where('digital', 2)->where('slug', $slug)->where('approved', 1)->first();
        $product_queries = ProductQuery::where('product_id', $detailedProduct->id)->where('customer_id', '!=', Auth::id())->latest('id')->paginate(10);
        $total_query = ProductQuery::where('product_id', $detailedProduct->id)->count();
        // Pagination using Ajax
        // if (request()->ajax()) {
        ...
        if ($detailedProduct != null && $detailedProduct->published) {
            $langs = PujariLang::where('status',1)->get();
            return view('frontend.pooja_product_details', compact('detailedProduct', 'product_queries', 'total_query', 'langs'));
        }
        else {
            abort(404);
        }
    ...
    public function contact_submit(Request $request)
    {
        $validatedData = Validator::make($request->all(),
            [
                ...
                'phone' => 'required|numeric|regex:/^[6-9][0-9]{9}$/|digits:10',
         ...
         else
         {
            try {
                ...
                $array['mail_subject'] = 'Message from '.env('APP_NAME').' - '.$_SERVER['SERVER_NAME'];
                $array['from'] = env('MAIL_FROM_ADDRESS');
                $array['content'] = '';
                $array['name'] = $request->get('name');
                $array['phone'] = $request->get('phone');
                $array['email'] = $request->get('email');
                $array['subject'] = $request->get('subject');
                $array['description'] = $request->get('message');
                Mail::to(env('MAIL_FROM_ADDRESS'))->queue(new ContactMailManager($array));
                $alertMessage = 'Details submitted succesfully. Our team will get in touch with you soon.';
                $success = true;
            } catch(\Exception $e) {
