...
if(!function_exists('get_store_previous_product')) {
    function get_store_previous_product($product_id) {
        $previous = Product::isApprovedPublished()
            ->where('digital','0')
            ->where('id', '<', $product_id)->max('id');
        return $previous;
    }
}

if(!function_exists('get_store_next_product')) {
    function get_store_next_product($product_id) {
        $next = Product::isApprovedPublished()
            ->where('digital','0')
            ->where('id', '>', $product_id)->min('id');
        return $next;
    }
}

if(!function_exists('get_astrology_previous_product')) {
    function get_astrology_previous_product($product_id) {
        $previous = Product::isApprovedPublished()
            ->where('digital','1')
            ->where('id', '<', $product_id)->max('id');
        return $previous;
...
if(!function_exists('get_astrology_next_product')) {
    function get_astrology_next_product($product_id) {
        $next = Product::isApprovedPublished()
            ->where('digital','1')
            ->where('id', '>', $product_id)->min('id');
        return $next;
...
if(!function_exists('get_pooja_previous_product')) {
    function get_pooja_previous_product($product_id) {
        $previous = Product::isApprovedPublished()
            ->where('digital','2')
            ->where('id', '<', $product_id)->max('id');
        return $previous;
...
if(!function_exists('get_pooja_next_product')) {
    function get_pooja_next_product($product_id) {
        $next = Product::isApprovedPublished()
            ->where('digital','2')
            ->where('id', '>', $product_id)->min('id');
        return $next;
...
if (!function_exists('user_default_address')) {
    function user_default_address() {
        if(Auth::user()->addresses != null)
            return Auth::user()->addresses->where('set_default', 1)->first();
        return '';
    }
}
if (!function_exists('add_some_days')) {
    function add_some_days($date) {
        // return $date;
        return Carbon::parse($date)->addDays(3)->format('d M, Y');
    }
}
if (!function_exists('get_shipment_order_details')) {
    function get_shipment_order_details($order_id) {
        return (new ShiprocketController)->order_details($order_id);
    }
}
if (!function_exists('get_shipment_tracking_details')) {
    function get_shipment_tracking_details($order_id) {
        return (new ShiprocketController)->track($order_id);
    }
}
