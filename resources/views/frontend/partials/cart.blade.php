...
if (auth()->user() != null) {
    ...
    $cart = \App\Models\Cart::where('user_id', $user_id)->with('product')->get();
} else {
    $temp_user_id = Session()->get('temp_user_id');
    if ($temp_user_id) {
        $cart = \App\Models\Cart::where('temp_user_id', $temp_user_id)->with('product')->get();
    }
}
                ...
                @if ($product != null)
                    <li class="list-group-item">
                        <span class="d-flex align-items-center">
                            <a href="{{ route((($product->digital == 1)?'astrology':(($product->digital == 2)?'pooja':'product')), $product->slug) }}"
