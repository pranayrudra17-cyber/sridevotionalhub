...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            <h5 class="mb-0 h6"><strong>{{translate('Upload Product Stock File')}}</strong></h5>
        </div>
        <div class="card-body">
            <form class="form-horizontal" action="{{ route('product_stock_upload') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="form-group row">
                    <div class="col-sm-9">
                        <div class="custom-file">
    						<label class="custom-file-label">
    							<input type="file" name="stock_file" class="custom-file-input" required>
    							<span class="custom-file-name">{{ translate('Choose File')}}</span>
    						</label>
    					</div>
                    </div>
                    <div class="col-sm-3">
                        <div class="">
                            <a href="{{ static_asset('download/product_stock_demo.xlsx') }}" download>
                                <button type="button" class="btn btn-success"><i class="las la-download"></i> {{ translate('Sample stock file')}}</button>
                            </a>
                        </div>
                    </div>
                </div>
